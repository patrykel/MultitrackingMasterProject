...
def get_submission_line(fill, run, first_reco_idx, last_reco_idx):
    submission_line = "sbatch " + \
                      "-J {2}-{3}_{1}_{0}_TOTEM-SINGLE ".format(fill, run, first_reco_idx, last_reco_idx) + \
                      "-N 1 " + \
                      "--ntasks-per-node=1 " + \
                      "--mem-per-cpu=4GB " + \
                      ...
                      "jobs/single_tracking_{}_{}_{}-{}.sh".format(fill, run, first_reco_idx, last_reco_idx)

    return submission_line


def get_job_filename(fill, run, first_reco_idx, last_reco_idx):
    return "generated/jobs/single_tracking_{}_{}_{}-{}.sh".format(fill, run, first_reco_idx, last_reco_idx)
